...
近期 **ClawHub** 访问量大时容易出现 **429 限流**；下列两种方式**不依赖 ClawHub 是否畅通**，请与 ClawHub 安装（§3.6）对照使用。
...
安装后仍需配置 **`plugins.entries.claw-ops`**（尤其是 **`config.accessToken`**），并视情况配置 **`tools.alsoAllow`**（见 §5.1）。最小闭环示例见下 **§3.7**；完整逐项命令见 **§4**。

### 3.7 安装后：命令行绑定 `accessToken`（环境变量引用，推荐）
...
**Token 环境变量引用**与 **§3.7** 中命令相同；本节补充 `enabled`、`baseUrl`、`hooks` 等其余项。
